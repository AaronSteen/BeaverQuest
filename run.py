#!/usr/bin/env python3
"""
Cross-platform launcher for Newgame - Normal Mode
...
def activate_venv_and_run():
    ...
    if check_venv_active():
        ...
        print("=" * 40)
        print("🎮 Launching Newgame in normal mode...")
        try:
            subprocess.run([sys.executable, "-m", "newgame.main"], check=True)
            print("✅ Newgame normal mode completed successfully.")
        except subprocess.CalledProcessError as e:
            print(f"❌ Failed to launch Newgame: {e}")
            sys.exit(1)
    ...
    if platform.system() == "Windows":
        ...
        if os.path.exists(powershell_script):
            ...
            full_command = shell_cmd + [
                f"& '{powershell_script}'; python -m newgame.main"
            ]
        elif os.path.exists(activate_script):
            shell_cmd = ["cmd", "/c"]
            full_command = shell_cmd + [
                f'"{activate_script}" && python -m newgame.main'
            ]
        else:
        ...
        shell_cmd = ["bash", "-c"]
        full_command = shell_cmd + [
            f"source '{activate_script}' && python -m newgame.main"
        ]
